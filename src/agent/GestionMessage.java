...
/**
 * Titre       : classe publique GestionMessage
 * Description : classe permettant de gérée les messages interne 
 * de l'agent et permet soit d'écrire sur la sortie standard le message, 
 * soit d'enregistrer le message dans un fichier log ou 
 * bien les 2 en même temps. Les messages sont de trois
 *  types différents : 	
 *  - [ERR] les erreurs du système - 2
 *  - [WAR] les erreurs non-bloquantes - 1
 *  - [MES] les messages de traitements - 0
 *  
 *   Les messages  sont écrit de la manière standardisé suivante : 
 *   [ERR] date - heure - classeEmettrice – Message
 *   
 *   La méthode Write permet d'écrire le message soit dans le 
 *   fichier log, soit en sortie standard
 *   La méthode gestionMessage permet de déclenchée 
 *   la méthode adaptée suivant les configuration de l'agent.
...
public class GestionMessage {
	 ...
	 * @param classeEmetrice String la classe qui appelle la méthode
	...
	private static String formatageMessage(int typeErreur, String classeEmetrice, String message ){
		...
		messageFormate = messageFormate + " - " + dateFormat()
										+ " - " + classeEmetrice
										+ " - " + message;
	...
	/**
	 * Méthode de formatage de la date du jour
	 * Attention c'est classe de gère pas l'éphéméride ;)
	 * 
	...
	private static void ecrireDansFichier(String message){
		...
		//on "catch" l exception ici si il y en a une, et on l affiche sur la console
			...
			System.out.println("[ERR] - " + dateFormat()
										+ " - GestionMessage - " +
										ioe );
			}
