    ...
    public static Connection connectionBDD() {
        ...
        if (Agent.CONFIG.getTypeBDD().equals("HSQL")) {
            try {
                ...
                // connexion en mémoire : toujours fraîche, pas de fichier résiduel
                connection = DriverManager.getConnection("jdbc:hsqldb:mem:" + Agent.CONFIG.getBaseBDD(),
                        Agent.CONFIG.getUserBDD(), Agent.CONFIG.getPassBDD());

                String createStatement = "CREATE TABLE IF NOT EXISTS BDD (\n"
                        + "  UID INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY,\n"
